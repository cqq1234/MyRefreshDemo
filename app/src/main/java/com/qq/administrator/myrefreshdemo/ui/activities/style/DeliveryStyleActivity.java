...
public class DeliveryStyleActivity extends AppCompatActivity implements AdapterView.OnItemClickListener {
    ...
    public void onItemClick(AdapterView<?> adapterView, View view, int position, long id) {
        switch (Item.values()[position]){
            case 默认主题:
                ...
                toolbar.setTitleTextColor(0xffbbbbbb);
                toolbar.setNavigationIcon(R.drawable.ic_arrow_back_gray_24dp);
                smartLayout.setPrimaryColors(0xfff0f0f0,0xffffffff);
                if (Build.VERSION.SDK_INT >= 21){
                    getWindow().setStatusBarColor(0xfff0f0f0);
                }
                if (Build.VERSION.SDK_INT >= 23) {
    ...
    private void setThemeColor(int colorPrimary,int colorPrimaryDark){
        ...
        smartLayout.setPrimaryColors(colorPrimary,android.R.color.white);
        if (Build.VERSION.SDK_INT >= 21) {
